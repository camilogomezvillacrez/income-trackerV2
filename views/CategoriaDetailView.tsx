...
export default function CategoriaDetailView({ catName }: Props) {
  ...
  const rows = (data.all_movs ?? []).filter((r) => r.category === catName && r.tipo === "gasto");
          ...
          {budget && cat && (
            <span
              style={{
                fontSize: "11px",
                fontWeight: 600,
                color: cat.total > budget ? "var(--red)" : "var(--green)",
                marginLeft: "8px",
              }}
            >
              {cat.total > budget ? "⚠ Superado" : "✓ Dentro del presupuesto"} · {fmt(budget)}
            </span>
          )}
              ...
              return (
                <div key={s.name} style={{ marginBottom: "10px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                    <span style={{ fontSize: "11px", color: "var(--text)", fontWeight: 500 }}>{s.name}</span>
